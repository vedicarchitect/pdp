...
The system SHALL expose `GET /api/v1/options/{underlying}/oi-series` returning OI change over time for multiple strikes. Query params SHALL include `expiry`, `interval` (5m, 15m, 1H, 1D), and optional `strikes` (default: top 10 by OI). The response SHALL be a time-series array suitable for multi-line charting. When `interval` is provided the endpoint SHALL downsample snapshots to keep only those at least `interval` minutes apart.
...
#### Scenario: Interval downsampling applied

- **WHEN** `GET /api/v1/options/NIFTY/oi-series?interval=1H` is called
- **THEN** the response contains only snapshots at least 60 minutes apart (not every 30-second poll snapshot)
...
The system SHALL expose `GET /api/v1/options/{underlying}/iv-history` returning `current_iv`, `iv_rank`, `iv_percentile`, `iv_high`, `iv_low`, and `lookback_days`. IV rank SHALL be computed as `(current - min) / (max - min)` over the lookback period (default 252 trading days). IV percentile SHALL be the percentage of historical days where IV was below the current IV. When fewer than 20 trading days of historical IV data exist, `iv_rank` and `iv_percentile` SHALL be returned as `null` alongside a `warning` message; all other fields (`current_iv`, `iv_high`, `iv_low`, `lookback_days`) SHALL still be populated.
