...
Decouple the monolithic API process into independently launchable, fault-isolated
processes — a stateless `pdp-api`, a stateful `pdp-engine` (market feed + strategies +
order execution), and a background `pdp-ops` worker — so that a failure in one
subsystem (e.g. a flaky Dhan feed connection or a slow warmup) degrades only that
subsystem instead of crashing the API, the dashboard, and the backtest console
together. Ticks/bars/indicator/position state move from in-process objects to
Redis-published snapshots and pub-sub/streams that the API bridges to WebSocket
clients, and live order placement is centralized in the engine via a durable Redis
Stream command channel with idempotency and a single kill-switch authority. This
satisfies the `repo-architecture` cloud-readiness requirement that the strategy
worker be a separately-launchable process decoupled from the API.
