# candlestick-ml-signals Specification

## Purpose
Offline-trained classical ML signals (LightGBM) over candlestick patterns, indicator snapshots,
and option-chain analytics. A single leakage-safe feature builder is shared between training and
inference so the model sees exactly the same feature vector offline and online. The signal is
served read-only via the indicator reader; strategies consume it without retraining.
