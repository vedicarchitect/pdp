...
Keep the test suite a trustworthy regression gate: `task test` SHALL pass in full and fail the
build the moment a real regression or an un-quarantined failure appears, so that every subsequent
change — especially to safety-critical mechanisms like the hard day-loss cap — is validated
against a suite whose green state actually means something, rather than one where failures are
tolerated as documented debt.
