# api-openapi-schema Specification

## Purpose
Keep the auto-generated OpenAPI schema (`/openapi.json`, `/docs`, `/redoc`) complete and typed,
so every JSON-returning route documents its response shape via a reusable Pydantic model instead
of a bare `dict`, and a regression test prevents the docs from silently going untyped again.
