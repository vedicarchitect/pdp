# strategy-crash-recovery Specification

## Purpose
Defines how strategies recover in-memory session state (open leg, day P&L baseline) from the
durable ledger when a process restarts mid-session, preventing ghost-position accumulation and
preserving risk-cap continuity.
