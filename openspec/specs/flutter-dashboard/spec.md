# flutter-dashboard Specification

## Purpose
The dashboard is the app's canonical home screen: a single-call aggregation of index/commodity
levels, portfolio snapshot, FII/DII flows, sentiment/news, expiry chips, and a user-editable
watchlist, kept live via the app's existing WebSocket connections rather than a new one, and built to
the house `domain/data/application/presentation` convention with honest per-section degradation.
