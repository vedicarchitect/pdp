# backtest-config-yaml Specification

## Purpose
Named YAML configuration files for backtest strategies stored under `backtest/configs/`, and the
`BACKTEST_DEFAULT_CONFIG` settings entry that points to the promoted config. Enables reproducible
named configs, a default no-arg `task backtest` experience, and a future API request-body seam.
