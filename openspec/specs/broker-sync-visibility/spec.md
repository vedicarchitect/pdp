...
Provides visibility into Dhan broker account state — holdings, positions and funds — mirrored
read-only into PostgreSQL, with a safe separation between lightweight intraday polling and the
authoritative end-of-day archival, and endpoints that let a client distinguish a disabled
subsystem from a genuinely empty account.
