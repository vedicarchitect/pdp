...
- [ ] 1.1 In `pdp/signals/bias.py`, remove the VWAP input and vote entirely:
...
- [ ] 1.2 The `score_bias` normalization divides by the sum of active weights — confirm the
...
- [ ] 1.3 Update `bias.py` unit tests: remove any test that passes `vwap=` into `BiasInputs`
...
- [ ] 2.1 In `pdp/strategies/directional_strangle.py`, remove the VWAP assembly: the
...
- [ ] 2.2 Remove the futures SID path now that nothing consumes it: delete
...
- [ ] 2.3 Grep the module for any remaining `vwap`/`futures_sid` reference and remove dead
  imports/helpers. The strategy must construct `BiasInputs` with no VWAP and resolve no
  futures SID.

## 3. Remove the backtest VWAP computation

- [ ] 3.1 In `pdp/backtest/strangle_loader.py`, remove the `_VWAP` running class (`:226-247`)
...
- [ ] 3.2 Confirm `strangle_sim` / `day_loader` pass no `vwap` into `score_bias` after the
  loader change — the shared `BiasInputs` no longer has the field, so a stale kwarg would be a
  hard error. Fix any call site.
- [ ] 3.3 Update the loader docstring (`:8`) to drop "tracks per-day VWAP".

## 4. Reconfirm the ₹1.35cr result holds

- [ ] 4.1 Re-run the multi-index sweep/backtest for NIFTY, BANKNIFTY, SENSEX over the same
...
- [ ] 4.2 Compare the no-VWAP results to the baseline. Because VWAP carried one low vote, the
...
- [ ] 4.3 Note the new figures in the change's completion summary so the roadmap's ₹1.35cr
  claim stays accurate.

## 5. Remove the "futures missing" radar family

- [ ] 5.1 In `pdp/backtest/completeness.py`, delete the `futures` family: remove `"futures"`
  ...
  futures source.
- [ ] 5.2 In `pdp/warehouse/coverage.py`, delete `_futures_family` (`:181-185`) and its wiring:
...
- [ ] 5.3 Grep both modules + `radar_for_date`/`radar_window` for any residual `futures`
...
- [ ] 5.4 Update any test in `tests/backtest/` that asserts a `futures` family in the radar
  output.

## 6. Daily backtest-vs-paper convergence check

- [ ] 6.1 Build a daily convergence report on the existing
...
- [ ] 6.2 Surface it as an endpoint (e.g. extend the vs-paper route with a `?cumulative=true`
...
- [ ] 6.3 The `cause` attribution must no longer ever emit `"futures missing"` (removed in
  task 5). Verify a day that previously reported only `"futures missing"` now reports `null`
  (genuinely unexplained) or a real cause.

## 7. Verify

- [ ] 7.1 `task test` green for `tests/signals/` (bias), `tests/strategies/`, `tests/backtest/`;
  `task lint` clean on all edited modules (pre-existing debt excluded).
- [ ] 7.2 Grep the whole `backend/` for `vwap`, `w_vwap`, `_futures_sid`, `futures_security_id`,
...
- [ ] 7.3 `GET /api/v1/coverage` radar output contains no `futures` family for any index.
- [ ] 7.4 The re-run sweep figures are recorded and the ₹1.35cr claim is reconfirmed (task 4).
- [ ] 7.5 `openspec validate backtest-paper-parity --strict`; archive on green.
