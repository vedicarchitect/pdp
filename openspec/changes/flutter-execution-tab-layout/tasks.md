...
## 5. Remaining
- [ ] 5.1 `app/CLAUDE.md`: record the convention — a widget branching on a width breakpoint gets a
...
- [ ] 5.2 Manual check on an Android device or emulator at phone width, confirming the index strip
      scales and the tab scrolls as one surface
- [ ] 5.3 Audit the other tabs for the same nested-scrollable and fixed-share-`Expanded` patterns
      (`grep -rn "ListView(" app/lib/features/*/presentation/`) — file follow-ups, do not fix here
- [ ] 5.4 Commit separately from `broker-sync-visibility`; the two changes share no scope
- [ ] 5.5 `openspec validate --strict flutter-execution-tab-layout` passes
