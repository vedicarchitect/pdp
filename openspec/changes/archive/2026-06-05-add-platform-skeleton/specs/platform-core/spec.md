...
#### Scenario: Readyz checks DB

- **WHEN** the database is reachable and `GET /readyz` is requested
- **THEN** the response is HTTP 200 with `{"status": "ready", "db": "ok", "redis": "ok"}`
- **AND WHEN** the database is unreachable
- **THEN** the response is HTTP 503 with `db: "error"` while `/healthz` continues to return 200
