...
- **THEN** a catch-all NotFound component is rendered
...
Each hook SHALL reconnect automatically with exponential back-off (1 s → 2 s → 4 s → 8 s, cap 30 s) when the WebSocket closes unexpectedly.
...
The mode is determined by reading the `X-Trade-Mode` response header from any API call made via TanStack Query. If the header is absent, the banner defaults to `PAPER`.
...
- **WHEN** an API response carries `X-Trade-Mode: live`
