...
- [x] 1.1 In `pdp/strategies/directional_strangle.py`, add an exit-fields helper
...
- [x] 1.2 P&L sign convention MUST match `_compute_unrealized` exactly: for a **short** leg
...
- [x] 1.3 Resolve `expiry` for the closed leg. `OpenLeg` has no `expiry` field today — add
...
- [x] 1.4 Resolve the human `symbol` via
...
- [x] 1.5 Wire `_leg_exit_fields` into every terminal close emit:
...
- [x] 1.6 `STOP_HALF` exit price is the current `ltp`; only `close_lots = leg.lots // 2` lots
...
- [x] 1.7 Do not change `_emit_event` (`:531`) itself — it already fans out to the daily log +
...
- [x] 2.1 Confirm `directional_strangle.state()` is the single P&L authority: it already
  returns `day_realized`, `day_unrealized`, `day_pnl`, `n_open_*`, `done_for_day`. Do not add a
  second P&L computation anywhere.
- [x] 2.2 The strangle console runs one strategy instance per index (NIFTY/BANKNIFTY/SENSEX).
...
- [x] 2.3 Add `GET /api/v1/strangle/pnl` (or extend `/strangle/stats`) returning a per-index
...
- [x] 3.1 Add a ledger reader `pdp/strategy/trade_ledger.py` with
...
- [x] 3.2 Each paired row: `{underlying, security_id, symbol, opt_type, strike, expiry, lots,
...
- [x] 3.3 Source events: prefer reading the day's JSONL at `logs/<strategy_id>/<date>.log`
  ...
  Mongo/PG store.
- [x] 3.4 Add `GET /api/v1/strangle/trades?strategy_id=&date=` in `pdp/strategy/routes.py`
...
- [x] 3.5 Unit-test `pair_trades` with a synthetic event list covering: (a) a clean
...
- [x] 4.1 Fix the realized-P&L bug in `pdp/journal/stats.py::compute_daily_stats`: realized
...
- [x] 4.2 Add a regression test in `tests/journal/test_stats.py`: a day with one open short
  ...
  minus charges.
- [x] 4.3 Make the Journal's trade source the enriched ledger: the Journal detail view returns
...
- [x] 4.4 Keep `record_fill` (`journal/service.py:80`) unchanged as the raw-fill capture; it is
...
- [x] 5.1 Add a shared currency formatter `app/lib/core/format/money.dart`:
...
- [x] 5.2 In `app/lib/features/journal/presentation/journal_screen.dart`, replace the
  hard-coded `\$` at lines 150, 151, 153, 272 with `inr(...)`. `Realized P&L` and `Charges`
  stats and the per-trade `@ price` all use `inr`.
- [x] 5.3 Render enriched trade rows from the ledger: each row shows `symbol`
...
- [x] 5.4 Update `app/lib/features/journal/domain/*` models + the journal data source to parse
...
- [x] 6.1 In `app/lib/features/manage/presentation/tabs/strategy_execution_tab.dart`, add a
...
- [x] 6.2 Add a compact day-P&L waterfall or sparkline per index (cumulative `pnl` of closed
...
- [x] 6.3 When a strategy's `done_for_day == true`, replace the live-MTM figure for that index
...
- [x] 6.4 The per-index P&L header on the Execution tab reads from `/strangle/pnl` (task 2.3)
  only — remove any dependency on `compute_daily_stats` / journal stats for live P&L.
- [x] 6.5 Update `app/lib/features/manage/domain/execution_models.dart` +
...
- [x] 7.1 Rename the shell nav destination `Holdings` → `Live Account (Dhan)` in
...
- [x] 7.2 Make the screen a two-tab surface: **Holdings** (equity/ETF — the existing
...
- [x] 7.3 Hard visual wall: a `● LIVE (manual)` badge (distinct accent, e.g. amber) on this
...
- [x] 7.4 Strictly read-only: no order/modify/cancel controls on the Live Account surface. Its
  numbers MUST NOT be added into the paper Dashboard / Execution / Journal P&L (they are a
  separate account book).
- [x] 7.5 Live-subscribed display MTM (backend): add an endpoint or startup hook that
...
- [x] 7.6 The Positions tab MTM streams over the existing portfolio/market WS (or a
  broker-positions WS if cleaner); reconnect with the standard exp-backoff `ws_client.dart`.

## 8. Verify

- [x] 8.1 Backend: `task test` green for `tests/strategy/`, `tests/journal/`,
...
- [x] 8.2 Manual/API check: `GET /api/v1/strangle/trades?date=today` returns entry→exit rows
...
- [x] 8.3 Flutter: Journal shows `₹` (no `$`), readable symbols, entry→exit round-trips with
...
- [x] 8.4 `cd app && flutter analyze && flutter test` green.
- [x] 8.5 `openspec validate live-pnl-trade-ledger-and-journal --strict`; archive on green.
