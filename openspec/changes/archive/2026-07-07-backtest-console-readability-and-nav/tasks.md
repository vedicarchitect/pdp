...
- [x] 1.1 In `pdp/warehouse/coverage.py::all_coverage` (`:291-305`), replace the serial
...
- [x] 1.2 In `underlying_coverage` (`:242-289`), parallelize the five family probes with
...
- [x] 1.3 Compute VIX coverage **once** for the whole request, not once per underlying (VIX is
...
- [x] 1.4 Use a **single shared/pooled Mongo client** for the whole coverage request instead of
  ...
  a per-family helper.
- [x] 1.5 Ensure the options `$group` (`gap_backfill.days_missing:373-379`, match on
...
- [x] 1.6 Target: `GET /api/v1/coverage` returns in < 2s for a 90-day window across all three
  indices. Measure and record the before/after latency in the completion summary.
- [x] 1.7 Backstop only: raise the Flutter `receiveTimeout` for the coverage call in the
...
- [x] 2.1 In `pdp/backtest/store.py`, compute a verdict for **single** runs too. Reuse the
  ...
  `None` (`:156`).
- [x] 2.2 Compute a verdict for the sweep **best-combo** doc as well (currently hard-coded
  `None` at `:592`): grade the top-ranked combo by the same thresholds so a sweep's headline
  isn't `--`.
- [x] 2.3 `promotion_state` remains `"none"` at creation (`:155`) and only flips through the
...
- [x] 2.4 Add/adjust a unit test in `tests/backtest/` asserting a single run with metrics above
...
- [x] 3.1 In `pdp/backtest/store.py::build_run_doc` (`:108-156`), add a top-level
  ...
  doc path.
- [x] 3.2 One-off backfill: add a small idempotent migration/script that sets top-level
...
- [x] 3.3 In `pdp/backtest/warehouse_routes.py::list_runs` (`:90-108`), add an `underlying`
...
- [x] 3.4 Add a per-index leaderboard resource, e.g. `GET /runs/leaderboard` (or
...
- [x] 4.1 In `app/lib/features/backtest/**`, render the Verdict column from the now-populated
  `verdict` field (PASS/REVIEW chip via theme tokens, never inline color); it must no longer
  show `--` for single runs.
- [x] 4.2 Default the Runs, Sweeps, and Coverage tabs to a **per-index grouped** layout
...
- [x] 4.3 Add a plain-English **leaderboard-by-index** card fed by the leaderboard resource
  (task 3.4): "NIFTY: best = ST(10,2)/15m, PF 5.72, PASS, promoted" — not raw combo JSON.
- [x] 4.4 Add layman explainer copy to the Sweeps + folds views: what a sweep is ("we tried N
...
- [x] 4.5 Update the backtest domain models + data source (live + mock per `AppConfig.useMock`)
  for the new top-level `underlying`, populated `verdict`, and leaderboard shape.

## 5. Flutter — remove duplicate nav

- [x] 5.1 In `app/lib/features/manage/presentation/manage_hub_screen.dart`, remove the
...
- [x] 5.2 Confirm the `TabController` length (if explicit) is updated to match the reduced tab
...
- [x] 5.3 Remove any now-unused imports of `StrategyExecutionTab` / `JournalTab` in the hub
  screen (leave the tab widgets themselves in place — they are still used by the left-nav routes).

## 6. Verify

- [x] 6.1 `task test` green for `tests/backtest/`, `tests/warehouse/`; `task lint` clean on all
  edited modules (pre-existing debt excluded). New verdict test from 2.4 passes.
- [x] 6.2 `GET /api/v1/coverage` for a 90-day, 3-index window returns in < 2s (measure); every
...
- [x] 6.3 Flutter: Coverage tab loads without a DioException; Verdict shows PASS/REVIEW (no
...
- [x] 6.4 `cd app && flutter analyze && flutter test` green.
- [x] 6.5 `openspec validate backtest-console-readability-and-nav --strict`; archive on green.
