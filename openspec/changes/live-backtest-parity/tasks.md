...
- [ ] 6.1 Run `python backtest_multiday.py --days 1 --start <today>` and `python scripts/backtest_compare.py --date <today>` on the next trading day after deploy; confirm paper trips >= backtest legs (or within 1-2 due to fill timing)
- [ ] 6.2 Confirm `pdp strategy heartbeat supertrend_short` shows `st_direction` non-None from bar 1
- [ ] 6.3 Check paper journal (`paper_journal` in MongoDB) and confirm `round_trips` on the next session matches backtest legs ± 1
