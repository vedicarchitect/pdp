...
@click.option("--format", type=click.Choice(["table", "json"]), default="table", help="Output format")
def option_chain_cmd(symbol: str | None, expiry: str | None, all_expiries: bool, format: str) -> None:
    ...
    show_option_chain(config, symbol, format, expiry=expiry, all_expiries=all_expiries)


@progress.command("greeks")
@click.option("--format", type=click.Choice(["table", "json"]), default="table", help="Output format")
def greeks_cmd(format: str) -> None:
    """Calculate and display Greeks for open positions."""
    from pdp.cli.progress.commands.greeks import show_greeks

    config = CLIConfig.from_env()
    show_greeks(config, format)
