...
| `atm_suite.py` | — | On-demand ATM CE/PE indicator rows for `/monitor` (`resolve_nifty_atm_option`/`build_atm_option_row`, NIFTY-only) plus a generic `resolve_atm_option(underlying, ...)` for any underlying, used by `atm_trend_read` — the ATM CE/PE 5m EMA/SuperTrend/PSAR trend read `directional_strangle.py` feeds into the bias engine's `w_atm` vote (`bias-ranking-multisignal`). Rolls up session-anchored bars via `pdp.market.bars.rollup_1m_bars`, builds one row per option side; each side opens its own DB session so the two sides can be gathered concurrently (`AsyncSession` can't run two queries on one connection) |
