...
| `intraday_run.py` | **Intraday-directional** multi-year runner — same quarter-chunked shape as `strangle_run.py`, drives `pdp/backtest/intraday_sim.py` |
| `walkthrough_run.py` | **EOD forensic report** — replays both engines for a date/range/trailing-N window and writes one `backend/backtest/manual/YYYY-MM-DD.md` per trading day + `INDEX.md`; `--date`/`--from`+`--to`/`--days`+`--start`, defaults to today (IST) with no args; refuses non-trading days and the NIFTY 2020-12-03→2023-01-05 blackout without `--force`. Run via `task eod:walkthrough` or `/eod:walkthrough` |
...
## `manual/` — EOD walkthrough reports

Committed (not git-ignored) per-day forensic markdown, one file per trading day
(`manual/YYYY-MM-DD.md`) generated by `walkthrough_run.py`, plus `manual/INDEX.md` (one summary
row per day: chg, day-type, both strategies' net, fill count, top finding IDs). Each file replays
both `directional_strangle` and `intraday_directional` for that day and ends in a ranked FINDINGS
section from `pdp/backtest/walkthrough_checks.py`'s invariant detectors — see
`pdp/backtest/CLAUDE.md` for the detector table. Re-running the same date is byte-identical apart
from the `generated_at` line, so a post-fix regeneration diffs cleanly.
