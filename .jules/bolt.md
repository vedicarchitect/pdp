...
## 2024-11-20 - Array.prototype.find() in React render loops
**Learning:** Using Array.prototype.find() inside render loops for large nested datasets (e.g., heatmaps or charts) creates O(N*M*K) performance bottlenecks.
**Action:** Pre-compute O(1) lookup structures (e.g., using Map within useMemo) instead of using find() inside render loops.
