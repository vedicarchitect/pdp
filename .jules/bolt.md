...
## 2026-06-23 - Precomputing Lookups in Nested Loops
**Learning:** In React components rendering large nested datasets (e.g., heatmaps), using Array.prototype.find() inside render loops creates O(N*M*K) performance bottlenecks.
**Action:** Pre-compute O(1) lookup structures (e.g., using Map within useMemo) instead to optimize rendering performance.
